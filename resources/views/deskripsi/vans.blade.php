      ...
      <div class="layout-container flex flex-col flex-1">
        <div class="flex flex-1 justify-center py-4 px-4 md:px-10">
          <div class="layout-content-container flex flex-col max-w-[1200px] flex-1">
            ...
            @foreach(['Vans Old Skool', 'Vans Authentic', 'Vans Slip-On', 'Vans Era', 'Vans Sk8-Hi', 'Vans Style 36', 'Vans Half Cab', 'Vans Rowley', 'Vans TNT', 'Vans AVE'] as $jenis)
                @php $hasData = false; @endphp
                @foreach($vans as $item)
                    @if($item->jenis_vans === $jenis)
                        ...
                        @if($item->jenis_vans === $jenis)
                            <div class="flex flex-col gap-2 pb-6 border border-gray-300 rounded-lg shadow-lg overflow-hidden transition-transform transform hover:scale-105 hover:shadow-xl">
                                <div class="flex justify-center">
                                    <img src="{{ asset('img/vans/'.$item->gambar) }}" alt="{{ $item->jenis_vans }}" class="w-full h-64 object-cover transition-transform duration-300 transform hover:scale-110">
                                </div>
                                <div class="p-4 bg-gray-50">
                                    <h3 class="text-[#111318] text-md font-semibold leading-normal">{{ $item->jenis_vans }}</h3>
